...
$262.agent.start(
   `$262.agent.receiveBroadcast(function (x_sab) {
      var report = [];
      var x = new Int8Array(x_sab); id3_R_t2 = Atomics.load(x, 0); report.push("id3_R_t2: "+id3_R_t2);
      $262.agent.report(report);
      $262.agent.leaving();
   })
   `);
...
$262.agent.start(
   `$262.agent.receiveBroadcast(function (x_sab) {
      var report = [];
      var x = new Int8Array(x_sab); id4_R_t3 = Atomics.load(x, 0); report.push("id4_R_t3: "+id4_R_t3);
      $262.agent.report(report);
      $262.agent.leaving();
   })
   `);
...
$262.agent.start(
   `$262.agent.receiveBroadcast(function (x_sab) {
      var report = [];
      var x = new Int8Array(x_sab); id5_R_t4 = Atomics.load(x, 0); report.push("id5_R_t4: "+id5_R_t4);
      $262.agent.report(report);
      $262.agent.leaving();
   })
   `);
